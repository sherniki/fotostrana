 ...
 * 
 */
public class ProxyManager implements StorageRequests {
  ...
  private String fileWithTimeChekedProxy = "data" + File.separator + "TimeCheckedProxy.data";
  /**
  ...
  protected void startDirectConnection() {
    ...
    Thread newThread = new Thread(direcionConnection, "Thread direction connection");
    newThread.start();
  }
  ...
  public void loadConfiguration() {
    fileCorrectHTTPproxy = ApplicationConfiguration.INSTANCE.getValue("configuration.Network.FileWithHTTP",
        fileCorrectHTTPproxy);
    fileCorrectSOCKSproxy = ApplicationConfiguration.INSTANCE.getValue("configuration.Network.FileWithSOCKS",
        fileCorrectSOCKSproxy);
    fileUncorrectProxy = ApplicationConfiguration.INSTANCE.getValue("configuration.Network.FileWithNotWorkingProxy",
        fileUncorrectProxy);
    fileWithTimeChekedProxy = ApplicationConfiguration.INSTANCE.getValue(
        "configuration.Network.ProxyChecker.FileWithTimeChekedProxy", fileWithTimeChekedProxy);

    proxyLogin = ApplicationConfiguration.INSTANCE.getValue("configuration.Network.ProxyLogin", proxyLogin);
    proxyPassword = ApplicationConfiguration.INSTANCE.getValue("configuration.Network.ProxyPassword", proxyPassword);
    proxyhubToken = ApplicationConfiguration.INSTANCE.getValue("configuration.Network.ProxyhubToken", proxyhubToken);
    limitDownloadProxy = ApplicationConfiguration.INSTANCE.getIntValue("configuration.Network.LimitDownloadProxy",
        limitDownloadProxy);
    countOfProxyClones = ApplicationConfiguration.INSTANCE.getIntValue("configuration.Network.CountCloneProxy",
        countOfProxyClones);
    DELAY_BETWEEN_UPDATES_PROXY = ApplicationConfiguration.INSTANCE.getIntValue(
        "configuration.Network.DelayBeetwenUpdateProxy", limitDownloadProxy);
  ...
  public boolean addProxy(String address, TypeProxy type) {
    ...
    if (newProxy != null)
      if ((!listAllProxy.contains(newProxy) && (!listNotWorkingProxy.contains(newProxy)))) {
  ...
  public int addListHTTPProxy(String[] listAdress, boolean createConnections) {
    ...
    newProxy.clear();
    Log.LOGGING.addNetworkLog("Добавлено " + countNewProxy + " новых прокси.", Log.getTypeMessage(countNewProxy));
  ...
  private void loadTimeCheked(String file) {
    ...
    try {
      ...
      try {
        timeChecked = (Map<String, Date>) objectInputStream.readObject();
      } catch (ClassNotFoundException e) {
        Log.LOGGING.addFileLog("Ошибка : " + e.getMessage(), Log.TYPE_NEGATIVE);
      }
    } catch (IOException e) {
      Log.LOGGING.addFileLog("Ошибка чтения файла: " + file + ". Ошибка : " + e.getMessage(), Log.TYPE_NEGATIVE);
    } finally {
      try {
        if (timeChecked == null)
  ...
  private void saveTimeCheked(String file) {
    ...
    try {
      ...
      Log.LOGGING.addFileLog("Ошибка записи в файл: " + file + ". Ошибка : " + e.getMessage(), Log.TYPE_NEGATIVE);
    } finally {
      try {
  ...
  public void loadListProxyFromFile(String file, TypeProxy type) {
    ...
    Log.LOGGING.addFileLog("Добавлено " + countNewProxy + " адресов.", Log.getTypeMessage(countNewProxy));
  }
  ...
  public AddressProxy getFreeProxy(int index) {
    if ((listFreeProxy.size() == 0) || (index > listFreeProxy.size()) || (index < 0))
      return null;
  ...
  public void updateProxy() {
    ...
    TaskDownloadProxy taskDownloadProxy = new TaskDownloadProxy(proxyLogin, proxyPassword);
