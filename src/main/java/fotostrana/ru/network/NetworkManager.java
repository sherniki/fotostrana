...
public class NetworkManager implements EventListener {
 ...
 static{
  ...
  public void loadConfiguration() {
    loadConfiguration(ApplicationConfiguration.INSTANCE);
  }

  public void loadConfiguration(ApplicationConfiguration config) {
    Integer configurationValue = null;
    configurationValue = config.getIntValue("configuration.Network.TimeOut");
    if ((configurationValue != null) && (configurationValue.intValue() > 0))
      Connection.TIME_OUT = configurationValue.intValue() * 1000;

    configurationValue = config.getIntValue("configuration.Network.TimeSleep");
    if ((configurationValue != null) && (configurationValue.intValue() > 0))
    ...
    if ((configurationValue != null) && (configurationValue.intValue() > 0)) {
      ...
      int countQueue = MAX_COUNT_CONNECTION / NUMBER_OF_PROXY_ON_ONE_QUEUE;
  ...
  public int addTheMaximumNumberOfConnections() {
    ...
    if (listConnections.size() < MAX_COUNT_CONNECTION) {
      ...
      Log.LOGGING.addNetworkLog("Создано " + result + " новых соединений.", Log.getTypeMessage(result));
    }
    if (listConnections.size() == 0) {
      Application.APPLICATION.handleEvent(new EventNoWorkingConnections());
    }
    return result;
  }
  ...
  public void deleteConnection(Connection connection) {
    ...
    if (listConnections.size() == 0) {
      Log.LOGGING.addNetworkLog("Все соединения закрыты", Log.TYPE_NEGATIVE);
      Application.APPLICATION.handleEvent(new EventNoWorkingConnections());
    }
  }
